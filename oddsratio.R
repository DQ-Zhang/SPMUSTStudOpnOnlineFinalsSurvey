...
for(m in 3:11){
  for(n in 3:11){
    if(m<n){
      ...
      #it must be subtracted & inverted to become a 2X2 contingency table
      con_tab=con_tab[3:2,2:3]
...
write.csv(dataset1,"cormatrix_3_11.csv")

#confd1(year1,2,3,4) vs confd2-indvar7
dataset2=data.frame(stringsAsFactors=FALSE)
for(var in 3:11){
  #first extract a 4x2 table from data of class and a variable
  fourbytwotab=table(survey[,2],survey[,var])
  fourbytwotab=fourbytwotab[2:5,2:3]
  for(i in 1:4){
    for(j in 1:4){
      if(i<j){
        #from that 4x2 table, six 2x2 contingency tables are derived.
        twobytwotab=fourbytwotab[c(i,j),]
        #analyze each 2x2 table
        result=epitools::oddsratio(twobytwotab,rev="c",verbose=TRUE)
        #extract information from result of analysis to be a row in the dataframe
        row=c(
          i,
          j,
          var,
